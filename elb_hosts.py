...
def get_all_running_instances():
    ...
    lb = alb_client.describe_load_balancers( LoadBalancerNames=['fligthsapielb-test'])
    alb_desc = lb['LoadBalancerDescriptions'][0];
    instances = alb_desc['Instances']
    instance_ips=[]
...
#	print instance['InstanceId']
	running_instance = \
            ec2.instances.filter(Filters=[{'Name': 'instance-state-name'
                                 , 'Values': ['running']}, {'Name':'instance-id', 'Values': ['i-0e0c19465cc5084de']}])
        print(running_instance)
        for inst in running_instance:
		print(inst)
       # for inst in running_instance:
